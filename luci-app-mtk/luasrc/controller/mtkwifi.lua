...
function index()
    ...
    --     end
    -- end
    if mtkwifi.exists("/lib/modules/ralink/hw_nat.ko") then 
       entry({"admin", "mtk", "hwnat"}, template("admin_mtk/mtk_hwnat"), _("Hardware NAT"))
       entry({"admin", "mtk", "hwnat", "toggle"}, call("hwnat_toggle")).leaf = true
       entry({"admin", "mtk", "hwnat", "status"}, call("hwnat_status"))
    end
...
local __mtkwifi_reload = function (devname)
    ...
    if map_help then
        local easymesh_applied_path = mtkwifi.__profile_applied_settings_path(mtkwifi.__easymesh_profile_path())
        os.execute("cp -f "..mtkwifi.__easymesh_profile_path().." "..easymesh_applied_path)
    end
end
...
function hwnat_toggle(enable)
    debug_write("debug", "hwnat_toggle "..tostring(enable))
    os.execute("mkdir -p /tmp/mtk/")
    if enable == "enable" then
        os.execute("hwnat-enable.sh >> /tmp/mtk/hwnat.log 2>&1")
    else
        os.execute("hwnat-disable.sh >> /tmp/mtk/hwnat.log 2>&1")
    end
    return luci.http.redirect(luci.dispatcher.build_url("admin", "mtk", "hwnat"))
end

function hwnat_status()
    local result = mtkwifi.read_pipe("hwnat -g 2>&1")
    result = result:gsub("<", "&lt;")
    result = result:gsub(">", "&gt;")
    http.write(tostring(result))
end
...
function save_easymesh_driver_profile(easymesh_cfgs)
    ...
    local map_app_cfgs = mtkwifi.load_profile(mtkwifi.__easymesh_map_profile_path())
    ...
    for _,profile in mtkwifi.__spairs(profiles, function(a,b) return string.upper(a) < string.upper(b) end) do
        ...
        driver_cfgs['MapEnable'] = easymesh_cfgs['MapEnable']
        driver_cfgs['MAP_Turnkey'] = tonumber(easymesh_cfgs['MapEnable']) == 1 and "1" or "0"
        if http.formvalue("TriBand") == "1" then
            if detected_5g == false and mtkwifi.band(driver_cfgs.WirelessMode) == "5G" then
                driver_cfgs['ChannelGrp'] = "0:0:1:1"
                detected_5g = true
            elseif detected_5g == true and mtkwifi.band(driver_cfgs.WirelessMode) == "5G" then
                driver_cfgs['ChannelGrp'] = "1:1:0:0"
            end
            map_app_cfgs['radio_band'] = "24G;5GH;5GL"
        elseif http.formvalue("TriBand") == "2" then
            ...
            map_app_cfgs['radio_band'] = "24G;5GL;5GH"
        end
        mtkwifi.save_easymesh_map_profile(map_app_cfgs)
        __mtkwifi_save_profile(driver_cfgs, profile, false)
    end
end

function map_cfg()
    local easymesh_cfgs = mtkwifi.load_profile(mtkwifi.__easymesh_profile_path())
    ...
    mtkwifi.save_easymesh_profile(easymesh_cfgs)
...
function apply_wifi_bh_priority(bhPriority2G, bhPriority5GL, bhPriority5GH)
    ...
    if r.status == "SUCCESS" then
        local easymesh_cfgs = mtkwifi.load_profile(mtkwifi.__easymesh_profile_path())
        easymesh_cfgs['BhPriority2G'] = bhPriority2G
        easymesh_cfgs['BhPriority5GL'] = bhPriority5GL
        easymesh_cfgs['BhPriority5GH'] = bhPriority5GH
        mtkwifi.save_easymesh_profile(easymesh_cfgs)
    end
    http.write_json(r)
end
...
function update_easymesh_bss(cfgs, bssInfoInp, isEdit)
    ...
    if isEdit == "1" then
        local editBssInfo = {}
        editBssInfo['ssid'] = http.formvalue('__EDIT_SSID')
