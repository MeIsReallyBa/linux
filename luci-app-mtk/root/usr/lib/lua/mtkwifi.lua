...
function mtkwifi.save_easymesh_profile(easymesh_cfgs)
    ...
    local fd = io.open(mtkwifi.__easymesh_profile_path(), "w")
    if not fd then return end
    ...
    for k,v in mtkwifi.__spairs(easymesh_cfgs, function(a,b) return string.upper(a) < string.upper(b) end) do
        fd:write(k.."="..v.."\n")
        debug_write("save_easymesh_profile:  key:"..k.."->value:"..v..",")
    end
    fd:close()
...
function mtkwifi.save_easymesh_profile_to_nvram()
    ...
    -- os.execute("cat "..dev1_profile_paths.." "..mtkwifi.__easymesh_profile_path().." "..mtkwifi.__easymesh_bss_cfgs_nvram_path().." > "..merged_easymesh_dev1_path.." 2>/dev/null")
    -- Comment or remove below line once above requirement is met.
    os.execute("cat "..dev1_profile_paths.." "..mtkwifi.__easymesh_profile_path().." > "..merged_easymesh_dev1_path.." 2>/dev/null")
...
function mtkwifi.save_easymesh_map_profile(easymesh_map_cfgs)
    if not easymesh_map_cfgs then
        return
    end
    local fd = io.open(mtkwifi.__easymesh_map_profile_path(), "w")
    if not fd then return end
    table.sort(easymesh_map_cfgs, function(a,b) return a<b end)
    for k,v in mtkwifi.__spairs(easymesh_map_cfgs, function(a,b) return string.upper(a) < string.upper(b) end) do
        fd:write(k.."="..v.."\n")
    end
    fd:close()
end

function mtkwifi.__easymesh_profile_path()
    return "/etc/mapd_cfg.txt"
end

function mtkwifi.__easymesh_map_profile_path()
    return "/etc/map_cfg.txt"
end
...
function mtkwifi.__easymesh_bss_cfgs_path()
    return "/etc/wts_bss_info_config"
end
...
function mtkwifi.get_easymesh_al_mac(devRole)
    local r = {}
    local mapd_app_cfgs = mtkwifi.load_profile("/etc/map_cfg.txt")
    if not mapd_app_cfgs then
        r['status'] = "Failed to load /etc/map_cfg.txt file!"
    else
        r['status'] = 'SUCCESS'
    ...
    for _,line in ipairs(mtkwifi.__lines(content)) do
        ...
        if string.byte(line) ~= string.byte("#") then
            ...
            if band then
                ...
                for token in string.gmatch(bssLineStr, "(%S+)%s?") do
                    tokIdx = tokIdx + 1
                    if tokIdx == 1 then
                        tmp[alMac][band][bssInfoIdx]['ssid'] = token
